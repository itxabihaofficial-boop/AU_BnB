...
const TeamDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
        try {
            // Check Lock
            const lockRes = await fetch('https://au-bnb-2.onrender.com/api/lock-status');
            const lockData = await lockRes.json();
            setIsLocked(lockData.isLocked);

            if (!lockData.isLocked) {
                // Fetch Challenge
                const response = await fetch(`https://au-bnb-2.onrender.com/api/my-challenge/${parsedUser.username}`);
  ...
  const fetchUploads = async (username) => {
      try {
          const res = await fetch(`https://au-bnb-2.onrender.com/api/my-uploads/${username}`);
  ...
  const handleUpload = async (e) => {
      ...
      try {
          const res = await fetch('https://au-bnb-2.onrender.com/api/upload', {
              method: 'POST',
              body: formData
          });
          const data = await res.json();

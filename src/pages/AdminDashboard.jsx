...
const AdminDashboard = () => {
  ...
  // 1. Initial Load
  useEffect(() => {
    fetch('https://au-bnb-2.onrender.com/api/lock-status')
      .then(res => res.json())
  ...
  const toggleLock = async (newStatus) => {
    ...
    setLoading(true);
    try {
        const res = await fetch('https://au-bnb-2.onrender.com/api/toggle-lock', {
            method: 'POST',
  ...
  const fetchSubmissions = async () => {
      try {
          const res = await fetch('https://au-bnb-2.onrender.com/api/all-uploads');
          const data = await res.json();
                            ...
                            filteredSubmissions.map((sub, idx) => (
                                <div key={idx} className="flex items-center justify-between bg-white/5 p-4 rounded-xl border border-white/5 hover:bg-white/10 transition-colors group">
                                    ...
                                    <div className="flex items-center gap-6">
                                        ...
                                        <a 
                                            href={`https://au-bnb-2.onrender.com/api/download/${sub.filename}`} 
